...
var _ = Describe("EgressServer", func() {
	...
	for _, manyToOneDiode := range []bool{false, true} {
		Describe(fmt.Sprintf("BatchedReceiver (manyToOneDiode=%t)", manyToOneDiode), func() {
	...
	Describe("BatchedReceiver with concurrent publishers", func() {
		It("delivers envelopes without a data race when multiple goroutines publish to the same subscriber (many-to-one diode)", func() {
			// Mirrors FanoutWriter: several goroutines call DataSetter.Set()
			// on the same subscription at once. Only the many-to-one diode
			// tolerates that; run with -race to confirm it does.
			ctx, cancel := context.WithCancel(context.Background())
			errors := make(chan error)
			defer func() {
				cancel()
			...
			subscriber := &spySubscriber{writers: 8}
			server := v2.NewEgressServer(
				subscriber,
				testhelper.NewMetricClient(),
				&metricemitter.Counter{},
...
type spySubscriber struct {
	...
	wait time.Duration
	// writers is the number of concurrent goroutines that call d.Set().
	// Defaults to 1 if unset, matching a single publisher.
	writers int
}
